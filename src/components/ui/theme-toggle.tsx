...
function getThemeButtonCopy(isHydrated: boolean, theme: ThemeMode) {
  if (!isHydrated) {
    return {
      label: "Cambiar tema",
      title: "Cambiar tema",
...
export function ThemeToggle() {
  ...
  const buttonCopy = getThemeButtonCopy(isHydrated, activeTheme);
  const Icon = isHydrated && activeTheme === "noir" ? SunIcon : MoonIcon;
  ...
  return (
    ...
    >
      <Icon />
      <span className="hidden sm:inline">{buttonCopy.text}</span>
    </button>
  );
}
