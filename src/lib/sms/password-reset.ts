...
export async function sendPasswordResetSms(input: {
  recipient: string;
  code: string;
}) {
  const response = await fetch("https://api.brevo.com/v3/transactionalSMS/sms", {
    ...
    body: JSON.stringify({
      ...
      type: "transactional",
      content: `RitmoHub: tu codigo para recuperar contrasena es ${input.code}. Expira en 10 minutos. Si no lo solicitaste, ignora este SMS.`,
    }),
    cache: "no-store",
  });
